---
name: {{SKILL_PREFIX}}-validate-security
description: Mechanically scan Arcana for credential patterns and unsafe Python constructs
when_to_use: Before committing changes that touched Python in `rites/` or any new authored content; before tagging a release; as a phase of `/arc-improve`; user mentions "security check", "credential scan", or "any committed secrets". Cheap and read-only - safe to run liberally.
user-invocable: true
