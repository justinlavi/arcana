...
This is a mechanical pattern scan that's distinct from `/arc-validate-naming` (which enforces snake_case on actual filenames). For *intelligent* semantic analysis (naming quality, organizational discoverability), use `/arc-grimoire-analyze-semantics`.
