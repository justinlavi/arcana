...
- Removed compatibility aliases and legacy shims by design.
- Added `.gitattributes` / `.editorconfig` guardrails for UTF-8 and LF line endings.
...
- `validate_grimoire_structure` - domain grimoire layout (root hub, `sources/`, `chapters/`, `log.md`); validates `grimoire.json` via the shared `_lib.load_manifest` (catches missing `name`, missing/invalid `namespace`, parse errors); enforces `.obsidian/app.json` with `newLinkFormat: "absolute"` so wikilinks cannot silently spawn recursive directory trees on Ctrl-click.
...
- Source SKILL.md files use `{{SKILL_PREFIX}}` and `{{ARCANA_PATH}}` placeholders resolved at registration time.
