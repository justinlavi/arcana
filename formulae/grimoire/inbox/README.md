...
Run `/grm-domain-ingest` (no argument sweeps `inbox/`). The skill walks the folder and, for each file, proposes a classification — confirmed in chat — then moves the file:
...
- Authored wiki content you've already written → `chapters/` directly (use `/grm-domain-create-chapter` if you want scaffolding).
