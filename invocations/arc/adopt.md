...
Use this when `/arc-sync library` reports an unmanaged directory: a folder under `~/grimoires/` that lacks `grimoire.json` and is therefore invisible to library and registration rites.

## Invocation

```
/arc-adopt [directory-name]
```
...
If the user passed `$directory`, use it. Otherwise run `/arc-sync library` to enumerate unmanaged directories and ask which one to adopt.
...
If the grimoire ships skills, also run `/grm-sync` from inside that grimoire or `/arc-sync skills` globally.
...
- Sync library: [[invocations/arc/sync|sync]]
- Create new grimoire: [[invocations/grm/create_grimoire|create grimoire]]
