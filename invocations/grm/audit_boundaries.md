...
Also runs as part of `/arc-improve` when auditing Arcana itself.
...
2. **No magical terminology in chapter prose.** Phrases like "perform the invocation" or "consult the formula" belong in Arcana docs only. Referencing Arcana by name (e.g. "run `/grm-health-check`") is fine.
...
- **Grimoire health check**: `/grm-health-check`
