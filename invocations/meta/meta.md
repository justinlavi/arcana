...
- **[help.md](help.md)** - `/arc-help`. Dynamically scans every invocation file and renders the catalog on-the-fly. Single source: the invocation files themselves.
- **[update_agent_block.md](update_agent_block.md)** - `/arc-agent-update`. Refresh the Grimoire block in user agent instruction files while preserving unrelated content.
...
- **[grimoire_directory_guard.md](grimoire_directory_guard.md)** - Shared precondition included via `!cat` by every `/arc-grimoire-*` skill that operates on the active grimoire (cwd). Single-sourced so the guard contract stays consistent across skills.
