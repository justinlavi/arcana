...
- **Magical boundary** — system terminology stays in Arcana; finding *content* for a grimoire stays domain-natural. See [[invocations/grm/audit_boundaries|audit boundaries]].
...
- applying or confirming edits (`/grm-health-check` confirm-then-apply, `/arc-improve` defers changes over ten files);
- log appends;
...
- Proven in: [[invocations/arc/quality/review_architecture|review architecture]]
- Consumers: [[invocations/grm/health_check|health check]], [[invocations/arc/improve_arcana|improve arcana]]
