...
The agent decides what to capture based on the immediate prior conversation. The user can scope it: "capture the comparison", "save the analysis we just did".
...
### 1. Identify the answer to capture
...
- **`source`** — only when filing a transcript of an external thing; usually `/grm-import` is the right skill for that.
...
1. Captured answers carry `authority: grimoire` unless they directly summarize a single source (in which case use `/grm-import`).
2. New page must be wired into a hub before considering the operation complete.
3. One log entry per captured answer.
...
- Page captured at `chapters/<chapter>/<slug>.md`
- Hub updated
- Log entry timestamp
