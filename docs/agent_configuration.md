...
The block never changes when grimoires are added or removed — those changes happen in the library, not the agent file. When Arcana itself changes the block, use `/arc-agent-sync-instructions` to refresh automatic registry targets or explicitly supplied agent instruction files while preserving unrelated user instructions.
...
- **All user-facing operations** (`/grm-*`, `/arc-library-*`, `/arc-agent-sync-skills`, `/arc-help`, `/arc-validate`) declare `when_to_use` so Claude can auto-suggest them.
...
- Run `/arc-agent-sync-instructions`. It compares existing agent instruction files against the canonical block and updates only the Grimoire section.
