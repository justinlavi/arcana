...
Invoke `/arc-sync-skills` for a global refresh of Arcana plus every installed grimoire. Invoke `/grm-sync-skills` from inside one grimoire to refresh Arcana plus that active grimoire only. The summoning rite runs the global registration for you on install.
...
- **All user-facing operations** (`/grm-*`, `/arc-library-*`, `/arc-sync-skills`, `/arc-help`, `/arc-validate`) declare `when_to_use` so Claude can auto-suggest them.
...
To register new or updated skills in the active grimoire, run `/grm-sync-skills`. To refresh all installed grimoires, run `/arc-sync-skills`.
...
- Run `/grm-sync-skills` from the active grimoire, or `/arc-sync-skills` for a global refresh. Then open a new agent session (Claude Code / Codex caches skill listings).
