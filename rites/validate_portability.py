...
def check_segments(rel_parts):
    """Return a list of violation descriptions for the path's segments."""
    issues = []
...
def main():
    ...
    for p in sorted(root.rglob("*")):
        ...
        if rel.parts and rel.parts[0] in SKIP_TOP_LEVEL:
            continue
        checked += 1
        for issue in check_segments(rel.parts):
            err(f"{rel}: {issue}")
            violations += 1
    ...
    if violations == 0:
        ...
        return 0
    print(f"Portability validation failed with {violations} violation(s)")
    print()
