...
Removes the .artifacts/ directory under Arcana's rites/. The validator
orchestrator (rites/validate.py) writes its transient run reports there; that
is the only place artifacts land, because rites/ is reserved for Arcana and
grimoires never carry one.

Usage:
    python3 rites/clean_artifacts.py              # clean
